...
const upcomingAppointments = [
  {
    ...
    type: "Cardiologie — Suivi insuffisance cardiaque",
    provider: "Dr. Sarah Chen",
    date: "2025-02-24",
    time: "09:30",
    location: "Portland Heart Center",
    urgent: true,
  },
  {
    id: "A2",
    ...
    type: "Gériatrie — Réévaluation cognitive",
    provider: "Dr. Helen Wu",
    date: "2025-02-25",
    time: "11:00",
    location: "Portland Geriatric Center",
    urgent: false,
  },
  {
    id: "A3",
    ...
    type: "Endocrinologie — Contrôle thyroïdien",
    provider: "Dr. Anna Volkov",
    date: "2025-02-26",
    time: "14:15",
    location: "Portland Endocrine Center",
    urgent: false,
  },
  {
    id: "A4",
    patientId: "P-001",
    patientName: "James Morrison",
    type: "Diabétologie — HbA1c & ajustement traitement",
    provider: "Dr. Lisa Patel",
    date: "2025-02-28",
    time: "10:00",
    location: "Portland Primary Care",
    urgent: false,
  },
  {
    id: "A5",
    patientId: "P-003",
    patientName: "Eleanor Whitfield",
    type: "Kinésithérapie — Bilan prévention chutes",
    provider: "Tom Bradley, PT",
    date: "2025-03-03",
    time: "08:45",
    location: "St. Vincent Medical Center",
    urgent: false,
  },
];
      ...
      <main className="container max-w-4xl py-12">
        <div className="mb-10">
          ...
          <p className="text-[13px] text-muted-foreground">
            Vue d'ensemble — rendez-vous à venir et dossiers patients.
          </p>
        </div>
        ...
        <div className="mb-10">
          <div className="clinical-label mb-4 flex items-center gap-2">
            <Calendar className="h-3.5 w-3.5" /> Prochains rendez-vous
          </div>
          <div className="border border-border rounded-xl overflow-hidden divide-y divide-border">
            {upcomingAppointments.map(apt => (
              <button
                ...
                className="w-full text-left px-5 py-4 flex items-center gap-5 hover:bg-muted/30 transition-colors group"
              >
                {/* Date column */}
                <div className="w-16 shrink-0 text-center">
                  <div className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
                    {formatDate(apt.date).split(' ').slice(0, 1)}
                  </div>
                  <div className="text-lg font-semibold text-foreground leading-tight">
                    {new Date(apt.date + "T00:00:00").getDate()}
                  </div>
                  <div className="text-[11px] text-muted-foreground">
                    {formatDate(apt.date).split(' ').slice(2).join(' ')}
                  </div>
                </div>
            ...
            {mockPatients.map(p => {
              ...
              return (
                <button
                  ...
                  className="w-full text-left px-5 py-4 flex items-center gap-4 hover:bg-muted/30 transition-colors group"
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      ...
                      {activeDx.length > 3 && (
                        <span className="text-[11px] text-muted-foreground self-center">+{activeDx.length - 3}</span>
                      )}
                    </div>
                  </div>
                  <ArrowRight className="h-4 w-4 text-muted-foreground/40 group-hover:text-foreground transition-colors shrink-0" />
