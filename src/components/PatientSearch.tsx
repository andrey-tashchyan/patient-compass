...
const PatientSearch = () => {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => { if (panelRef.current && !panelRef.current.contains(e.target as Node)) setIsOpen(false); };
  ...
  const handleSelect = (patient: Patient) => { setQuery(""); setIsOpen(false); navigate(`/patient/${patient.patient_id}`); };
          ...
          <div ref={panelRef} className="w-full max-w-lg bg-card border border-border rounded-xl shadow-2xl overflow-hidden">
            <div className="flex items-center gap-3 px-4 border-b border-border">
              ...
              <input ref={inputRef} type="text" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={handleKeyDown}
                placeholder="Name, MRN…" className="flex-1 h-12 text-sm bg-transparent text-foreground placeholder:text-muted-foreground focus:outline-none" />
            ...
            <div className="max-h-[50vh] overflow-y-auto">
              {results.length === 0 ? (
                ...
                <div className="py-1">
                  {!query.trim() && <div className="px-4 pt-2 pb-1"><span className="clinical-label">All patients</span></div>}
                  {results.map((p, i) => {
                    ...
                    return (
                      <button key={p.patient_id} onClick={() => handleSelect(p)}
                        ...
                        <div className="w-9 h-9 rounded-full bg-muted flex items-center justify-center shrink-0"><User className="h-4 w-4 text-muted-foreground" /></div>
