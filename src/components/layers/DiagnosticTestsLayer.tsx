...
const DiagnosticTestsLayer = ({ tests }: { tests: DiagnosticTest[] }) => (
  <CollapsibleLayer
    title="Examens diagnostiques"
    icon={<ClipboardList className="h-4 w-4" />}
    badge={<span className="clinical-badge-info">{tests.length}</span>}
  >
    {tests.length === 0 ? (
      <p className="text-[13px] text-muted-foreground">Aucun examen diagnostique.</p>
    ) : (
      <div className="space-y-3">
        {tests.map((t, i) => (
          <div key={i} className="data-card space-y-2">
            ...
            {t.ordered_by && <div className="text-[12px] text-muted-foreground">Prescrit par {t.ordered_by}</div>}
            <div className="text-[13px] text-data-value leading-relaxed">{t.findings}</div>
            <div className="text-[12px] text-primary font-medium">Interprétation : {t.interpretation}</div>
          </div>
        ))}
      </div>
    )}
